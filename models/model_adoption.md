...
```python
        ...
        cos, sin = position_embeddings
        # remove the previous four lines and add this three lines
        # 修改后仅先对Q注入RoPE
        query_states = apply_rotary_pos_emb_single(query_states, cos, sin)
        # 把未注入RoPE的K, V以及RoPE的参数博保存到PackKVCache
        cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
        key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
...
```python
        ...
        if not isinstance(past_key_values, PackKVCache):
            past_key_values = PackKVCache(input_ids.shape[0], self.config.num_attention_heads, self.config.hidden_size // self.config.num_attention_heads, self.config.num_hidden_layers)
