...
class CactiConfig:
    ...
    def __init__(self):
        ...
        self.baseline_config = [
            ...
            '\n',
            # BRISK-KV RTL does not currently implement SRAM ECC.  Enabling it
            # here would silently charge area/energy for hardware that is not
        ...
        self.config_option = {}
        self.config_option['cache_size'] = {'string': '-size (bytes) ',
        ...
        self.config_option['line_size'] = {'string': '-block size (bytes) ',
                                            'option': [8, 16, 32, 64, 128],
                                            'default': 64}

        # Unit for IO_bus_width is bit.
        self.config_option['IO_bus_width'] = {'string': '# Wordwidth (bit)\n' + '-output/input bus width ',
                                               'option': [4, 8, 16, 24, 32, 64, 128],
                                               'default': 128}

        self.config_option['associativity'] = {'string': '-associativity ',
                                                'option': [0, 1, 2, 4],
                                                'default': 1}

        self.config_option['rd_wr_port'] = {'string': '-read-write port ',
                                             'option': [0, 1, 2, 3, 4],
                                             'default': 0}

        self.config_option['ex_rd_port'] = {'string': '-exclusive read port ',
                                             'option': [0, 1, 2, 3, 4],
                                             'default': 2}

        self.config_option['ex_wr_port'] = {'string': '-exclusive write port ',
                                             'option': [0, 1, 2, 3, 4],
                                             'default': 2}

        self.config_option['single_rd_port'] = {'string': '-single ended read ports ',
                                                 'option': [0, 1, 2, 3, 4],
                                                 'default': 0}

        self.config_option['bank_count'] = {'string': '-UCA bank count ',
                                             'option': [1, 2, 4, 8, 16],
                                             'default': 1}

        self.config_option['technology'] = {'string': '-technology (u) ',
                                             'option': [0.022, 0.032, 0.045, 0.065, 0.090, 0.180],
                                             'default': 0.022}

        self.config_option['mem_type'] = {'string': '-cache type ',
                                           'option': ['"cache"', '"ram"', '"main memory"'],
                                           'default': '"ram"'}
        
        self.config_option['mem_cell_type'] = {'string': '-Data array cell type - ', 
    ...
    def change_default_value(self, new_config: Dict):
        for option, new_value in new_config.items():
            self.config_option[option]['default'] = new_value
