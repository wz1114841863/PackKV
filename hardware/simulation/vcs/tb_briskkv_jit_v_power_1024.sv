...
/**
 * Representative maximum-geometry activity workload for BRISK-KV.
 *
 * This is intentionally separate from tb_briskkv_jit_v.sv: the 64-token
 * smoke test remains the compact exact-value functional golden, while this
 * workload exercises 1024 tokens x 128 features for power annotation.
 */
module tb_briskkv_jit_v_power_1024;
  ...
  initial begin
    wave_file = "outputs/jit_v_1024t_128f_attention";
    ...
    begin
      if ((activity_phase == phase || activity_phase == "combined") &&
        ...
        if ($test$plusargs("VCD"))
          $dumpvars(0, tb_briskkv_jit_v_power_1024.dut);
`ifdef BRISKKV_VCS
        if ($test$plusargs("VPD"))
          $vcdpluson(0, tb_briskkv_jit_v_power_1024.dut);
`endif
...
`endif
  ...
  task automatic drive_write_pair(input integer pair);
    begin
      ...
      // Match BriskKvCycleBenchmarkSpec's deterministic 0..1 Q12 workload.
...
`endif
    ...
    $display("BRISK-KV 1024x128 POWER PASS: phase=%s write_cycles=%0d attention_cycles=%0d overlap=%0b checksum=%0d activity_start_ns=%0d activity_stop_ns=%0d activity_duration_ns=%0d",
             activity_phase, write_cycles, attention_cycles, overlap_seen,
             output_checksum, activity_start_time, activity_stop_time,
