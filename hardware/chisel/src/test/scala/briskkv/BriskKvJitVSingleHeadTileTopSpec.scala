...
class BriskKvJitVSingleHeadTileTopSpec
    extends AnyFreeSpec
    with Matchers
    with ChiselSim {
  ...
  private def fixedWeights(logits: IndexedSeq[Long]): IndexedSeq[Int] = {
  ...
  "Selectable dual/shared-decoder JIT-V single-head tile" in {
    ...
    val weights = fixedWeights(logits)
    ...
    val gateWriterClock =
      ...
      dut.io.attentionTag.poke(121.U)
      ...
      while (!resultSeen && cycles < 200000) {
        ...
        val loadedWeights =
          dut.io.attentionProgress.jitV.loadedWeightPackets.peek().litValue
        val vLaunched = dut.io.attentionProgress.vLaunched.peek().litToBoolean
        if (vLaunched && loadedWeights < TokenCount / 16)
          overlapSeen = true
        ...
        if (dut.io.result.valid.peek().litToBoolean) {
          dut.io.result.bits.tag.expect(121.U)
