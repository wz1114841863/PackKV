...
no_extend_benchmarks = ["coqa", "gsm8k", "winogrande", "squadv2", "squad_completion"]
...
def add_to_map(pair, setting_map):
    if unified_hash(pair) in setting_map:
        assert (
            pair == setting_map[unified_hash(pair)]
        ), "config with same hash but different value"
    else:
        setting_map[unified_hash(pair)] = pair


# 在生成超参数的搜索空间
channel_quant_k_quant_scale_rels = [0.01] + gen_quant_rels(
    channel_quant_k[0], channel_quant_k[1], 6
)
channel_quant_k_ext_quant_scale_rels = [0.01] + gen_quant_rels(
    channel_quant_k_ext[0], channel_quant_k_ext[1], 18
)
token_quant_k_quant_scale_rels = [0.01] + gen_quant_rels(
    token_quant_k[0], token_quant_k[1], 6
)
token_quant_k_ext_quant_scale_rels = [0.01] + gen_quant_rels(
    token_quant_k_ext[0], token_quant_k_ext[1], 18
)
token_quant_v_quant_scale_rels = [0.01] + gen_quant_rels(
    token_quant_v[0], token_quant_v[1], 6
)
token_quant_v_ext_quant_scale_rels = [0.01] + gen_quant_rels(
    token_quant_v_ext[0], token_quant_v_ext[1], 18
)
...
for model_name in model_list:
    for benchmark in benchmarks:
        for quant_scale in (
            channel_quant_k_ext_quant_scale_rels
            if benchmark not in no_extend_benchmarks
            else channel_quant_k_quant_scale_rels
        ):
        ...
        for quant_scale in (
            token_quant_k_ext_quant_scale_rels
            if benchmark not in no_extend_benchmarks
            else token_quant_k_quant_scale_rels
        ):
            ...
            add_to_map((benchmark, config), setting_map)
        for quant_scale in (
            token_quant_v_ext_quant_scale_rels
            if benchmark not in no_extend_benchmarks
            else token_quant_v_quant_scale_rels
        ):
